...
Before I jump into Handler business, let me make a statement.
...
Are both ways identical? They are not identical but close. A minor difference is View.post will queue request if View is not attached and handler will immediately post to UI thread queue. Using the wrong way might lead to difficult to reproduce bugs.
...
When I started Android, I googled and copied the following code:
...
You know what is happening under the hood.
