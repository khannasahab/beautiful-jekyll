...
2. No one is stopping you to use the property without initialising it, compiler does not warn you. If you use it before intialised, you get `UninitializedPropertyAccessException`
...
### What if you need to intialise a val property later?
...
{: .box-plain}
In the above example, `heading` is a val but it will be asigned lazily when first accessed. As lazy is just returning a delegator, it can be used with both `var and val`
