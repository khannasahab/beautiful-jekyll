...
{: .box-bordered}
This is one of the are Java is really ugly to look at. We need to use either `+` operator to concatenate strings OR use string formatter. Former option is mostly used in common places like printing logs.<br/><br/>Kotlin offers a neat way. You can print value of the object inside a string liternal using `$`. Check the snippet below, both are same:
